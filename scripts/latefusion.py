...
def main(params):
    ...
    *list_data, labels, gene_names, target_surv = load_TIPIT_multimoda(
    ...
    if config["classifier"]["type"] == "xgboost":
        base_clf = Pipeline(steps=[('xgboost', CustomXGBoostClassifier(**config["classifier"]["args"]))])
    ...
    pipe_rna = ("RNA",
                Pipeline(steps=[('omics_imputer', CustomOmicsImputer(site_feature=-1))
    ...
    def _fun_repeats(r, disable_infos):
        ...
        late_clf = LateFusionClassifier(estimators=[pipe_clinical, pipe_radiomics, pipe_pathomics, pipe_rna],
        ...
        for fold_index, (train_index, test_index) in tqdm(enumerate(cv.split(np.zeros(len(y)), y)),
            ...
            for c, idx in enumerate(indices):
                ...
                X_thresholds[test_index, c] = clf.find_logrank_threshold(X_train, target_surv_train, estim_ind=idx)
        ...
        if config["permutation_test"]:
            ...
            for prm in range(config["n_permutations"]):
                ...
                for fold_index, (train_index, test_index) in tqdm(enumerate(cv.split(np.zeros(len(y)), y)),
                                                                  ...
                                                                  total=cv.get_n_splits(np.zeros(len(y))),
                                                                  disable=disable_infos):
                    X_train, yshuffle_train, X_test = X[train_index, :], yshuffle[train_index], X[test_index, :]
                    clf = clone(late_clf)
                    clf.fit(X_train, yshuffle_train)

                    for c, idx in enumerate(indices):
                        X_perm[test_index, c] = clf.predict_proba(X_test, estim_ind=idx)[:, 1]

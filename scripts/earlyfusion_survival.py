...
def main(params):
    ...
    *list_data, labels, gene_names, target_surv = load_TIPIT_multimoda(
    ...
    if config["survival_model"]["type"] == "RF":
        estimator = Pipeline(steps=[('RF', CustomRandomForest(**config["survival_model"]["args"]))])
    ...
    def _fun_repeats(r, disable_infos):
        ...
        for fold_index, (train_index, test_index) in tqdm(enumerate(cv.split(np.zeros(len(y)), y)),
            ...
            cv_inner = CensoredKFold(n_splits=10, shuffle=True, random_state=np.random.seed(r))
            for c, models in enumerate(names):
                t = {model: transformers[model] for model in models.split('+')}
                early_surv = EarlyFusionSurvival(estimator=estimator,
                                                 transformers=t,
                ...
                X_thresholds[test_index, c] = early_surv.find_logrank_threshold(X_train, y_train)

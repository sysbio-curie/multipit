...
def main(params):
    ...
    *list_data, labels, gene_names, target_surv = load_TIPIT_multimoda(
    ...
    c, rad, p, o = list_data[0].shape[1], list_data[1].shape[1], list_data[2].shape[1], list_data[3].shape[1]
    dic_modalities = {"clinicals": np.arange(0, c),
    ...
    if config["classifier"]["type"] == "xgboost":
        estimator = CustomXGBoostClassifier(**config["classifier"]["args"]) \
            if len(config["classifier"]["args"]) > 0 else CustomXGBoostClassifier()
    elif config["classifier"]["type"] == "LR":
        estimator = Pipeline(steps=[('final_imputer', SimpleImputer(strategy="constant", fill_value=0)),
                                    ("LR", LogisticRegression(**config["classifier"]["args"]))])

    if config["classifier"]["type"] == "xgboost":
        transformers = {"clinicals": None, "pathomics": None, "radiomics": None,
                        "RNA": CustomOmicsImputer(site_feature=-1)}
    else:
        transformers = {"radiomics": Pipeline(steps=[("scaler", RobustScaler()),
                                                     ("rad_impute",
                                                      CustomImputer(**config["radiomics_data"]["imputation"]))]),
                        "RNA": Pipeline(steps=[('omics_process', CustomOmicsImputer(site_feature=-1)),
                                               ('scaler', RobustScaler()),
                                               ('omics_impute', CustomImputer(**config["rna_data"]["imputation"]))]),
                        "pathomics": Pipeline(steps=[("scaler", RobustScaler()),
                                                     ("path_impute",
                                                      CustomImputer(**config["pathomics_data"]["imputation"]))]),
                        "clinicals": Pipeline(steps=[("scaler", RobustScaler()),
                                                     ("clinicals_impute",
    ...
    def _fun_repeats(r, disable_infos):
        ...
        for fold_index, (train_index, test_index) in tqdm(enumerate(cv.split(np.zeros(len(y)), y)),
            ...
            cv_inner = StratifiedKFold(n_splits=10, shuffle=True, random_state=np.random.seed(r))
            for c, models in enumerate(names):
                t = {model: transformers[model] for model in models.split('+')}
                early_clf = EarlyFusionClassifier(estimator=estimator,
                                                  transformers=t,
                                                  modalities={model: dic_modalities[model] for model in
                                                              models.split('+')},
                                                  cv=cv_inner,
                ...
                X_thresholds[test_index, c] = early_clf.find_logrank_threshold(X_train, target_surv_train)

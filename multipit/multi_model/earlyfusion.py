...
class EarlyTransform(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        for modality, transformer in self.transformers.items():
            ...
            self.transformed_modalities_[modality] = np.arange(
                n, n + X_transformed.shape[1]
            )
...
class EarlyFusionClassifier(BaseEstimator):
    ...
    def _collect_preds(self, X, y, train, test):
        ...
        transformer = EarlyTransform(
            modalities=self.modalities,
            transformers={
                moda: _clone(transform) for moda, transform in self.transformers.items()
            },
        )
        X_train_transformed = transformer.fit(X_train, y_train).transform(X_train)
        if self.select_features:
            select_transform = CustomSelection(
                threshold=self.threshold_select,
            ...
            X_train_transformed = select_transform.fit(
                X_train_transformed, y_train, modalities=modalities
        ...
        mask_test = (
            np.isnan(X_test_transformed).sum(axis=1) == X_test_transformed.shape[1]
        )
        if self.select_features:
    ...
    def fit(self, X, y):
        ...
        self : object
            Returns the instance itself.
        """
        self.earlytransform_ = EarlyTransform(
            modalities=self.modalities,
            transformers={
                moda: _clone(transform) for moda, transform in self.transformers.items()
            },
        ...
        if self.select_features:
            self.select_transform_ = CustomSelection(
                threshold=self.threshold_select,
                max_corr=self.max_corr,
                max_number=self.max_features,
            ...
            for i, mod in enumerate(
                self.earlytransform_.transformed_modalities_.values()
            ):
                modalities += [i] * len(mod)
            X_transformed = self.select_transform_.fit(
                X_transformed, y, modalities=np.array(modalities)
            ).transform(X_transformed)
        mask = (
            np.isnan(X_transformed).sum(axis=1) == X_transformed.shape[1]
        )  # samples without any modality !
        if self.balance_features:
            fweights = []
            for _, val in self.earlytransform_.transformed_modalities_.items():
                fweights += [1 / len(val)] * len(val)
            self.fitted_estimator_ = clone(self.estimator).fit(
                X_transformed[~mask, :], y[~mask], feature_weights=fweights
            )
        else:
            self.fitted_estimator_ = clone(self.estimator).fit(
                X_transformed[~mask, :], y[~mask]
            )
        ...
        if self.calibration:  # only on samples with a least one modality !
            ...
            collected_predictions = parallel(
                delayed(self._collect_preds)(X=X, y=y, train=train, test=test)
            ...
            self.calibrator_ = LogisticRegression(class_weight="balanced").fit(
                predictions[~drop_empty_mask], y[~drop_empty_mask]
            )
        return self
    ...
    def predict_proba(self, X):
        ...
        if self.calibration:
            probas = self.calibrator_.predict_proba(
                self.fitted_estimator_.predict_proba(X_transformed)[:, 1].reshape(-1, 1)
            )
        else:
...
class EarlyFusionSurvival(BaseEstimator):
    ...
    def predict(self, X):
        ...
        if self.calibration:
            preds = (
                self.fitted_estimator_.predict(X_transformed) - self.calibrate_mean_
            ) / self.calibrate_std_
        else:
